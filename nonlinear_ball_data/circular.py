...
class BallBox:
    def __init__(self, dt=0.2, res=(32, 32), init_pos=(3, 3), init_std=0, wall=None, gravity=(0.0, 0.0)):
        ...
        self.dd = 20
        self.gravityStrength = 5.0e5
        self.centre = self.initial_position

    def planetGravity(self, body, gravity, damping, dt):
        # Gravitational acceleration is proportional to the inverse square of
        # distance, and directed toward the origin. The central planet is assumed
        # to be massive enough that it affects the satellites but not vice versa.
        sq_dist = body.position.get_dist_sqrd(self.centre)
        g = (
            (body.position - pymunk.Vec2d(*self.centre))
    ...
    def fire(self, angle=50, velocity=20, radius=3):
        ...
        ball.body.velocity_func = self.planetGravity

        # Set the box's velocity to put it into a circular orbit from its
        # starting position.
        r = ball.body.position.get_distance(self.centre)
        v = math.sqrt(self.gravityStrength / r) / r #angular velocity for given radius and strength
        v *= 0.8 #for elliptical orbit
        ball.body.velocity = (ball.body.position - pymunk.Vec2d(*self.centre)).perpendicular() * v
        # Set the box's angular velocity to match its orbital period and
        # align its initial angle with its position.
        ball.body.angular_velocity = v
        ball.body.angle = math.atan2(ball.body.position.y, ball.body.position.x)
    ...
    def run(self, iterations=20, sequences=500, angle_limits=(0, 360), velocity_limits=(10, 25), radius=3,
        ...
        if save:
            images = np.empty((sequences, iterations, int(self.res[0]/4), int(self.res[1]/4)), dtype=np.float32)
        ...
        for s in tqdm(range(sequences)):
            ...
            for i in range(iterations):
                ...
                if save == 'png':
                    ...
                    images[s, i] = pygame.surfarray.array2d(self.screen).swapaxes(1, 0).astype(np.float32)[0::4, 0::4] / (2**24 - 1)
...
if __name__ == '__main__':
    ...
    dt = 0.04
    cannon = BallBox(dt=dt, res=(32*scale, 32*scale), init_pos=(16*scale, 16*scale), init_std=10*scale, wall=None)
    cannon.run(delay=1/dt, iterations=200, sequences=64, radius=3*scale, angle_limits=(0, 360),
               velocity_limits=(10.0*scale, 15.0*scale), filepath='nonlinear_ball_data/ellipticallong', save='npz')

    print("Generating test sequences")
    np.random.seed(5678)
    cannon = BallBox(dt=dt, res=(32*scale, 32*scale), init_pos=(16*scale, 16*scale), init_std=10*scale, wall=None)
    cannon.run(delay=1/dt, iterations=40, sequences=0, radius=3*scale, angle_limits=(0, 360),
               velocity_limits=(10.0*scale, 15.0*scale), filepath='nonlinear_ball_data/elliptical_test2', save='npz')
